...
// 1. Busca os dados da planilha pública do Google Sheets
async function buscarCardapioGoogleSheets() {
  try {
    const url = `https://docs.google.com/spreadsheets/d/${process.env.GOOGLE_SHEET_ID}/gviz/tq?tqx=out:csv`;
    const response = await axios.get(url);
    const parsedData = Papa.parse(response.data, { header: true, skipEmptyLines: true });
    
    // Filtra apenas os itens marcados como 'Sim' na coluna Disponivel
    return parsedData.data.filter(
      item => item.Disponivel && item.Disponivel.trim().toLowerCase() === 'sim'
    );
  } catch (error) {
...
// Rota de teste (Acesse a URL do Render no navegador do celular para ver o cardápio em JSON)
app.get('/', async (req, res) => {
  const cardapio = await buscarCardapioGoogleSheets();
  res.json({ status: 'Servidor no ar!', total_itens: cardapio.length, cardapio });
});

// 2. Validação do Webhook (Exigido pelo painel do WhatsApp Meta)
...
// 3. Recebe a mensagem do cliente, consulta a planilha e responde com o Gemini
app.post('/webhook', async (req, res) => {
  res.sendStatus(200); // Responde imediatamente para a Meta não reenviar a mensagem
  ...
  try {
    ...
    if (message && message.type === 'text') {
      ...
      // Busca o cardápio atualizado na planilha
      const cardapio = await buscarCardapioGoogleSheets();

      // Envia o contexto da planilha + mensagem para o Gemini
...
`;
      ...
      // Dispara a resposta de volta para o WhatsApp do cliente
      await axios.post(
